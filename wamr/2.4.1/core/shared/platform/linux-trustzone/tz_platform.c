...
os_malloc(unsigned size)
{
    ...
    DMSG("Friedrich: TEE_Malloc(size=%u)", size);
#endif
...
os_mremap(void *old_addr, size_t old_size, size_t new_size)
{
    ...
    ret = tee_remap(new_size, old_addr);
    if (ret == NULL) {
        os_printf("os_mremap(old_addr=%p, old_size=%zu, new_size=%zu) failed: %u.", old_addr,
                  old_size, new_size, ret);
    }
    else {
        DMSG("os_mremap(old_size=%lu, new_size=%lu, old_addr=%p, new_addr=%p) OK.", old_size, new_size, old_addr, ret);
    }
...
os_mmap(void *hint, size_t size, int prot, int flags, os_file_handle file)
{
    ...
    // ret = TEE_Malloc(aligned_size, TEE_USER_MEM_HINT_NO_FILL_ZERO);
    if (ret == NULL) {
