 ...
 * While this function should return the time since the REE started, this
 * function is mostly used for code profiling in the runtime (i.e., used for
 * time difference), so it is assumed ok to provide the UNIX timestamp.
 *
 * Since the original attempt is to return the time since the REE started, the
 * value returned by this function is by essence untrusted.
 */
uint64
...
clock_gettime(clockid_t clock_id, struct timespec *tp)
{
    ...
    // The implemnentation has been changed in the kernel driver to retrieve the
    // monotonic time
    if (clock_id != CLOCK_MONOTONIC) {
        EMSG("clock_gettime does not support a clock_id different from "
             "CLOCK_MONOTONIC.");
        return -1;
    }
