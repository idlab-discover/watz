                          ...
                          uint64 commit_size)
{
    ...
    if (mapped_mem) {
        ...
        LOG_ERROR("(Friedrich) mremap linear_memory does os_mmap %d", new_size);
#endif
    ...
    // Friedrich: Enable the following to make the Orin crash
    // if (os_mprotect(new_mem, commit_size, MMAP_PROT_NONE) != 0) {
...
#ifdef FRIEDRICH_DEBUG
        os_printf("WASM_MEMORY 1644 CALLED");
#endif
...
#endif
    {
        // NOTE(Friedrich): map_size = 3 * 64kB
