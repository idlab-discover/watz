...
loader_malloc(uint64 size, char *error_buf, uint32 error_buf_size)
{
    ...
    os_printf("loader_malloc CALLED: %d", size);
#endif
...
#ifdef FRIEDRICH_DEBUG
    os_printf("loader_mmap CALLED 2: %d", size);
#endif
    if (!(mem = os_mmap(NULL, size, map_prot, map_flags,
                        os_get_invalid_handle()))) {
        set_error_buf(error_buf, error_buf_size, "allocate memory failed");
...
#ifdef FRIEDRICH_DEBUG
    os_printf("AOT_LOADER 2684 CALLED");
#endif
...
#if !defined(BH_PLATFORM_LINUX) && !defined(BH_PLATFORM_LINUX_SGX) && !defined(BH_PLATFORM_LINUX_TRUSTZONE) \
...
#ifdef FRIEDRICH_DEBUG
        os_printf("AOT_LOADER 3916 CALLED");
#endif
    ...
    for (i = 0; i < module->data_section_count; i++) {
        ...
        if (!strcmp(data_section->name, ".rdata")
            ...
            || !strncmp(data_section->name, ".rodata.str",
                        strlen(".rodata.str"))) {
#ifdef FRIEDRICH_DEBUG
            os_printf("AOT_LOADER 3939 CALLED");
#endif
