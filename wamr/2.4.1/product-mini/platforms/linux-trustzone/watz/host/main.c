...
start_wasm(tee_ctx *ctx, char *wasm_path, char *arg)
{
    ...
    // We start measuring just before the invocation of the code that launches
    // the WASM app in the TA
...
#ifdef PROFILING_LAUNCH_TIME
    // The measurement finished in the WASM app. We are free to take some time
    // to display numbers here. We print the recorded start at the first column
    // of data. The second one will be printed when the output buffer is
    // printed.
