...
TA_HashWasmBytecode(wamr_context *ctx)
{
    ...
    res = TEE_DigestDoFinal(operation_handle, ctx->wasm_bytecode,
                            ctx->wasm_bytecode_size, ctx->wasm_bytecode_hash,
                            &digest_len);
    if (res != TEE_SUCCESS) {
    ...
    if (digest_len != expected_digest_len) {
        EMSG("The hash size does not correspond to the expected value (actual: "
             "%d; expected: %d).",
