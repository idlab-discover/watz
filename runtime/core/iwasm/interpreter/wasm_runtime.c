                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    printf("[RUNTIME] Memory instantiate heap size: %d\n", heap_size);
#endif
    WASMModule *module = module_inst->module;
    ...
    if (memory_idx == 0) {
        ...
        if (init_page_count == max_page_count && init_page_count == 1) {
            ...
            if (init_page_count == max_page_count && init_page_count == 0) {
                ...
                //NOTE(Friedrich) Hier wordt heap_size van 8096 naar resp. 65536 en 64512 veranderd
                // inc_page_count = 1
                // init_page_count = 2
#ifdef FRIEDRICH_DEBUG
                printf("405 %d\n", init_page_count);
...
#ifdef FRIEDRICH_DEBUG
    LOG_VERBOSE("Linear memory allocated!");
#endif
...
#ifdef FRIEDRICH_DEBUG
        printf("Memory instantiate i: %d, max_page_count: %d, bytes/page: %d, init page: %d\n", i, max_page_count, module->memories[i].num_bytes_per_page, module->memories[i].init_page_count);
#endif
        if (!(memories[mem_index] = memory_instantiate(
                  module_inst, parent, memory, mem_index,
...
#ifdef FRIEDRICH_DEBUG
    printf("DONE WITH MEMORIES_INSTANTIATE\n");
#endif
