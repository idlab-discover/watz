                          ...
                          uint64 commit_size)
{
    ...
    if (mapped_mem) {
        ...
        os_printf("WASM_MEMORY 1501 CALLED Size: %d -> %d", old_size, new_size);
#endif
        new_mem = os_mremap(mapped_mem, old_size, new_size);
    }
    else {
#ifdef FRIEDRICH_DEBUG
        os_printf("WASM_MEMORY 1504 CALLED Size: %d", new_size);
#endif
...
#ifdef FRIEDRICH_DEBUG
    os_printf("WASM_MEMORY 1520 CALLED Size: %d", commit_size);
#endif
