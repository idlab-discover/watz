...
void CheckStatus(const OrtApi *g_ort, OrtStatus *status) {
  if (status != NULL) {
...
int main(int argc, char *argv[]) {
  ...
  // line: OrtSessionOptionsAppendExecutionProvider_CUDA(sessionOptions, 0);
  ...
  int64_t *input_node_dims; // simplify... this model has only 1 input node {1,
                            // 3, 224, 224}. Otherwise need vector<vector<>>
  ...
  //   CheckStatus(g_ort, g_ort->CastTypeInfoToTensorInfo(typeinfo,
  //   &tensor_info)); ONNXTensorElementDataType type; CheckStatus(g_ort,
  //   g_ort->GetTensorElementType(tensor_info, &type)); printf("Input %zu :
  //   type=%d\n", i, type);
  //
  ...
  const char *output_node_names[13] = {"softmaxout_1"};
  ...
  CheckStatus(g_ort, g_ort->CreateTensorWithDataAsOrtValue(
                         memory_info, input_tensor_values,
                         input_tensor_size * sizeof(float), input_node_dims, 4,
                         ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_tensor));
  int is_tensor;
  CheckStatus(g_ort, g_ort->IsTensor(input_tensor, &is_tensor));

  g_ort->ReleaseMemoryInfo(memory_info);

  // score model & input tensor, get back output tensor
  OrtValue *output_tensor = NULL;
  CheckStatus(g_ort, g_ort->Run(session, NULL, input_node_names,
                                (const OrtValue *const *)&input_tensor, 1,
                                output_node_names, 1, &output_tensor));
  CheckStatus(g_ort, g_ort->IsTensor(output_tensor, &is_tensor));
