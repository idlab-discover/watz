...
static void benchmark_roundtrip(tee_context *tee, uint32_t iterations) {
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    res = TEEC_InvokeCommand(&tee->session, TA_COMMAND_ROUNDTRIP, &op,
                             &err_origin);
    clock_gettime(CLOCK_REALTIME, &after);

    if (res != TEEC_SUCCESS) {
      printf("TEEC_InvokeCommand failed with code %#x origin %#x", res,
             err_origin);
    }
...
static void benchmark_tatime(tee_context *tee, uint32_t iterations) {
  ...
  for (int i = 0; i < iterations; i++) {
    res = TEEC_InvokeCommand(&tee->session, TA_COMMAND_GETREETIME, &op,
                             &err_origin);
