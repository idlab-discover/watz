...
void testset_main(void){
  // All the tests have been refactored into separate functions.
}
...
int speedtest1_main(int argc, char **argv){
  ...
  sqlite3_db_config(g.db, SQLITE_DBCONFIG_RESET_DATABASE, 1, 0);
  ...
  // Make sure the database is clear and the settings applied,
  // (required for WAMR, otherwise the internal number of pages of SQLite is not set to zero).
  sqlite3_exec(g.db, "VACUUM", 0, 0, 0);
  sqlite3_db_config(g.db, SQLITE_DBCONFIG_RESET_DATABASE, 0, 0);

  sz = n = g.szTest*500;
  zNum[0] = 0;
  maxb = roundup_allones(sz);

  BENCHMARK_START(exp100);
  speedtest1_launch100();
  BENCHMARK_STOP(exp100);
  fprintf(stderr, "100,%lld\n", timespec_to_micro(exp100));

  BENCHMARK_START(exp110);
  speedtest1_launch110();
  BENCHMARK_STOP(exp110);
  fprintf(stderr, "110,%lld\n", timespec_to_micro(exp110));

  BENCHMARK_START(exp120);
  speedtest1_launch120();
  BENCHMARK_STOP(exp120);
  fprintf(stderr, "120,%lld\n", timespec_to_micro(exp120));

  BENCHMARK_START(exp130);
  speedtest1_launch130();
  BENCHMARK_STOP(exp130);
  fprintf(stderr, "130,%lld\n", timespec_to_micro(exp130));

  BENCHMARK_START(exp140);
  speedtest1_launch140();
  BENCHMARK_STOP(exp140);
  fprintf(stderr, "140,%lld\n", timespec_to_micro(exp140));

  BENCHMARK_START(exp142);
  speedtest1_launch142();
  BENCHMARK_STOP(exp142);
  fprintf(stderr, "142,%lld\n", timespec_to_micro(exp142));

  BENCHMARK_START(exp145);
  speedtest1_launch145();
  BENCHMARK_STOP(exp145);
  fprintf(stderr, "145,%lld\n", timespec_to_micro(exp145));

  BENCHMARK_START(exp150);
  speedtest1_launch150();
  BENCHMARK_STOP(exp150);
  fprintf(stderr, "150,%lld\n", timespec_to_micro(exp150));

  BENCHMARK_START(exp160);
  speedtest1_launch160();
  BENCHMARK_STOP(exp160);
  fprintf(stderr, "160,%lld\n", timespec_to_micro(exp160));

  BENCHMARK_START(exp161);
  speedtest1_launch161();
  BENCHMARK_STOP(exp161);
  fprintf(stderr, "161,%lld\n", timespec_to_micro(exp161));

  BENCHMARK_START(exp170);
  speedtest1_launch170();
  BENCHMARK_STOP(exp170);
  fprintf(stderr, "170,%lld\n", timespec_to_micro(exp170));

  BENCHMARK_START(exp180);
  speedtest1_launch180();
  BENCHMARK_STOP(exp180);
  fprintf(stderr, "180,%lld\n", timespec_to_micro(exp180));

  BENCHMARK_START(exp190);
  speedtest1_launch190();
  BENCHMARK_STOP(exp190);
  fprintf(stderr, "190,%lld\n", timespec_to_micro(exp190));

  //BENCHMARK_START(exp200);
  //speedtest1_launch200();
  //BENCHMARK_STOP(exp200);
  //fprintf(stderr, "200,%lld\n", timespec_to_micro(exp200));

  BENCHMARK_START(exp210);
  speedtest1_launch210();
  BENCHMARK_STOP(exp210);
  fprintf(stderr, "210,%lld\n", timespec_to_micro(exp210));

  BENCHMARK_START(exp230);
  speedtest1_launch230();
  BENCHMARK_STOP(exp230);
  fprintf(stderr, "230,%lld\n", timespec_to_micro(exp230));

  BENCHMARK_START(exp240);
  speedtest1_launch240();
  BENCHMARK_STOP(exp240);
  fprintf(stderr, "240,%lld\n", timespec_to_micro(exp240));

  BENCHMARK_START(exp250);
  speedtest1_launch250();
  BENCHMARK_STOP(exp250);
  fprintf(stderr, "250,%lld\n", timespec_to_micro(exp250));

  BENCHMARK_START(exp260);
  speedtest1_launch260();
  BENCHMARK_STOP(exp260);
  fprintf(stderr, "260,%lld\n", timespec_to_micro(exp260));

  BENCHMARK_START(exp270);
  speedtest1_launch270();
  BENCHMARK_STOP(exp270);
  fprintf(stderr, "270,%lld\n", timespec_to_micro(exp270));

  BENCHMARK_START(exp280);
  speedtest1_launch280();
  BENCHMARK_STOP(exp280);
  fprintf(stderr, "280,%lld\n", timespec_to_micro(exp280));

  BENCHMARK_START(exp290);
  speedtest1_launch290();
  BENCHMARK_STOP(exp290);
  fprintf(stderr, "290,%lld\n", timespec_to_micro(exp290));

  BENCHMARK_START(exp300);
  speedtest1_launch300();
  BENCHMARK_STOP(exp300);
  fprintf(stderr, "300,%lld\n", timespec_to_micro(exp300));

  BENCHMARK_START(exp310);
  speedtest1_launch310();
  BENCHMARK_STOP(exp310);
  fprintf(stderr, "310,%lld\n", timespec_to_micro(exp310));

  BENCHMARK_START(exp320);
  speedtest1_launch320();
  BENCHMARK_STOP(exp320);
  fprintf(stderr, "320,%lld\n", timespec_to_micro(exp320));

  BENCHMARK_START(exp400);
  speedtest1_launch400();
  BENCHMARK_STOP(exp400);
  fprintf(stderr, "400,%lld\n", timespec_to_micro(exp400));

  BENCHMARK_START(exp410);
  speedtest1_launch410();
  BENCHMARK_STOP(exp410);
  fprintf(stderr, "410,%lld\n", timespec_to_micro(exp410));

  BENCHMARK_START(exp500);
  speedtest1_launch500();
  BENCHMARK_STOP(exp500);
  fprintf(stderr, "500,%lld\n", timespec_to_micro(exp500));

  BENCHMARK_START(exp510);
  speedtest1_launch510();
  BENCHMARK_STOP(exp510);
  fprintf(stderr, "510,%lld\n", timespec_to_micro(exp510));

  BENCHMARK_START(exp520);
  speedtest1_launch520();
  BENCHMARK_STOP(exp520);
  fprintf(stderr, "520,%lld\n", timespec_to_micro(exp520));

  BENCHMARK_START(exp980);
  speedtest1_launch980();
  BENCHMARK_STOP(exp980);
  fprintf(stderr, "980,%lld\n", timespec_to_micro(exp980));

  BENCHMARK_START(exp990);
  speedtest1_launch990();
  BENCHMARK_STOP(exp990);
  fprintf(stderr, "990,%lld\n", timespec_to_micro(exp990));
...
int main(int argc, char* argv[])
{
  ...
  int res;
  int size = 19 * 1024 * 1024;
  void* database_memory = malloc(size);
  ...
  int speedtest1_argc = 8;
  char *speedtest1_argv[] = { NULL, "-cachesize", "2048", "-pagesize", "4096", "--memdb", "--size", "60" };

  res = speedtest1_main(speedtest1_argc, speedtest1_argv);
