...
for iterations in {1..500}
do
    sshpass -p "$BM_BOARD_PASS" ssh -p $BM_SSH_PORT $BM_BOARD_USER@$BM_BOARD_HOSTNAME "watz $wasm_heap_size $initial_linear_memory_size /root/polybench_all.aot 2>&1" | tee -a $LOGS_DIR/polybench/tee-qemu.csv
done
