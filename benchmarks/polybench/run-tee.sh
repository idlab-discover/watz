...
do
    ...
    do
        echo "$local_file,$(sshpass -p "$BM_BOARD_PASS" ssh $BM_BOARD_USER@$BM_BOARD_HOSTNAME "vedliot_attester $wasm_heap_size polybench-wasm/$local_file 2>&1")" | tee -a $LOGS_DIR/polybench/tee.csv
    done
done
